...
\cite{yangFindingUnderstandingBugs2011}.
...
behaviours.
\cite{sangiorgiIntroductionBisimulationCoinduction2011} In
...
\cite{xiaInteractionTreesRepresenting2020} Similarly, MLIR has an
...
\cite{keizerImplementingDefinitionalCo}, which is based on earlier
...
\cite{traytelCategoryTheoryBased}, which follow the same
coinduction-as-a-library approach, and languages like Coq
\cite{gimenezTutorialRecursiveTypes1998}\cite{gimenezApplicationCoinductiveTypes1996}
...
\cite{furerQuotientsBoundedNatural2022} There are still new
...
property \footnote{ The subject reduction property states that reducing
  a program does not change its type}.~\cite{sozeauCorrectCompleteType}
